    ...
    try {
      await _auth.currentUser!.delete().then((value) {
        ...
        context.read<DeleteUserCubit>().setDeleteUser(userId: context.read<AuthCubit>().getUserId(), context: context).then((value) {
          ...
          for (int i = 0; i < auth.AuthProvider.values.length; i++) {
            if (auth.AuthProvider.values[i].name == context.read<AuthCubit>().getType()) {
              context.read<AuthCubit>().signOut(auth.AuthProvider.values[i]).then((value) {
                Navigator.of(context).pushNamedAndRemoveUntil(Routes.login, (route) => false);
              });
            }
          }
        ...
        return Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: <Widget>[
          ...
          Column(
            ...
            children: [
              ...
              CustomTextLabel(
                  text: 'firstAccLbl', textStyle: Theme.of(context).textTheme.titleMedium?.copyWith(color: UiUtils.getColorScheme(context).primaryContainer, overflow: TextOverflow.ellipsis)),
            ],
          ),
        ]);
      }
